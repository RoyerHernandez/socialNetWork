...
func HandlerFunc() {
	...
	router.HandleFunc("/updateProfile", middlew.DBCheck(middlew.JWTValidate(routers.ModifyProfile))).Methods("PUT")
	router.HandleFunc("/insertTweet", middlew.DBCheck(middlew.JWTValidate(routers.RecordTweet))).Methods("POST")
